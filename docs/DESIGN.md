# Snare — Design Document (v2, Rust-first)

> **Snare** — an open-source, AI-native web security proxy, written in **Rust**.
...
2. **Tri-frontend จากแกนเดียว** — TUI (ratatui) · Web (WASM/SPA) · Desktop (Tauri) ใช้ `snare-core` ร่วมกัน ไม่เขียน logic ซ้ำ.
...
```
        ...
        │   snare-tui (ratatui)      snare-web (WASM SPA)      snare-desktop     │
        │   สาย terminal/lazygit      เปิดในเบราว์เซอร์          (Tauri, native)   │
        └───────────────┬───────────────────┬───────────────────┬──────────────┘
                        │  snare-client (SDK) — REST/WS/gRPC ไปยัง daemon เดียว    │
                        ▼                    ▼                    ▼
        ┌───────────────────────────── snared (daemon) ─────────────────────────┐
        │  API surface:  axum(REST + WS)  ·  MCP(stdio + Streamable HTTP, rmcp)  │
        ├───────────────────────────────────────────────────────────────────────┤
        │                       snare-core  (library crate)                      │
        ...
        │  │ + blob store (content-addressed)│   │  ← impl: snare-engine (Rust) │ │
        │  │ adapter → Postgres (team mode)  │   │    hudsucker/hyper/rustls    │ │
        │  └─────────────────────────────────┘   └──────────────────────────────┘│
        └───────────────────────────────┬───────────────────────────────────────┘
   [Browser / client / Stealth-Browser MCP] ──▶ snare-engine (data-plane) ──▶ [Target]
```

**หลักการ:** logic ทั้งหมดอยู่ใน `snare-core` (Rust library). `snared` ห่อ core ด้วย transport. Frontends ทั้ง 3 เป็น **เปลือกบาง** ที่คุยกับ `snared` ผ่าน `snare-client` SDK ตัวเดียว → ไม่มี logic ซ้ำ, พฤติกรรมตรงกันทุกหน้า, และ **ต่อ remote daemon ได้** (TUI/Desktop เชื่อม snared ที่รันบนเซิร์ฟเวอร์ได้แบบ Caido [1]).

**โหมด single-binary:** `snare-tui`/`snare-desktop` สามารถ embed `snare-core` ตรง ๆ (ไม่ต้องมี daemon แยก) สำหรับใช้เครื่องเดียวแบบเบาสุด — เพราะ core เป็น library.
...
- ใช้ `snare-client` → ต่อ daemon local หรือ remote.

### 5.2 Web — `snare-web`
- SPA ต่อ `snared` (REST + WS). เปิดในเบราว์เซอร์, host บนเซิร์ฟเวอร์แล้วต่อไกลได้ (แบบ Caido [1]).
...
### 5.3 Desktop — `snare-desktop` (Tauri [5])
- ห่อ `snare-web` ใน **native webview** (WebView2/WebKit/GTK) + backend Rust = bundle < 10MB, RAM ~30–50MB [5].
- ฝัง/สั่ง `snared` ให้ในตัว, จัดการ CA trust, เมนู native, auto-update.

> ทั้ง 3 หน้าใช้ core+API เดียวกัน → พฤติกรรม/ข้อมูลตรงกันเป๊ะ.

---

## 6. Component — Proxy Engine (`snare-engine`, data-plane)
...
# project / realtime
POST /api/v1/projects · /{id}/open · /{id}/export {format:snare|har|json}
...
```
→ parser (crate `snare-core::httpql`) แปลงเป็น SQL WHERE + FTS5.
...
```
**Resources:** `snare://flow/{id}`, `snare://issue/{id}` ให้ agent อ้างหลักฐาน. **โหมด read-only** จำกัด active tool ตอนยังไม่อนุญาต.

---

## 11. Feature Parity Matrix (Burp → Snare, all-Rust)

| Burp | Snare | เฟส | หมายเหตุ |
|---|---|---|---|
...
```
snare/                        # cargo workspace root (Cargo.toml [workspace])
├── docs/                     DESIGN.md, ADRs, API.md, HTTPQL.md
├── crates/
│   ├── snare-core/           domain: capture, repeater, intruder, scanner, rules, httpql, storage port
│   ├── snare-engine/         proxy data-plane (hudsucker/hyper/rustls/rcgen)  ← impl ProxyEngine
│   ├── snare-store-sqlite/   SQLite WAL + writer actor + blob store
│   ├── snare-store-postgres/ scale-out adapter
│   ├── snare-mcp/            MCP server (stdio + streamable-http, rmcp)
│   ├── snare-client/         client SDK (REST/WS/gRPC) — ใช้โดย TUI/Desktop
│   ├── snared/               daemon: axum(REST+WS) + MCP + serve web assets  (bin)
│   ├── snare-tui/            ratatui frontend  (bin)
│   └── snare-desktop/        Tauri app         (bin)
├── web/                      snare-web SPA (Leptos/Dioxus หรือ TS) → build → served by snared/tauri
...
|---|---|---|
| **0 · Skeleton** | cargo workspace, `snare-core` traits, `snared` เปล่า, CI (fmt/clippy/test), gen-CA | `cargo build` ผ่านทุก crate, CI เขียว |
| **1 · Core Loop** | snare-engine(hudsucker) + capture + SQLite + REST/WS + **snare-tui** + MCP(stdio) | จับ HTTPS จริง, TUI เห็น flow สด + Repeater, `proxy_list_flows`/`repeater_send` จาก Claude, restart แล้วงานไม่หาย |
| **2 · Attack + Web** | Intruder(4โหมด, parallel) + Match&Replace + Decoder/Comparer + Sitemap + HTTPQL เต็ม + **snare-web** | Intruder 500 payload+grep, M&R สด, Web UI ใช้ได้เท่า TUI |
| **3 · AI + Passive + Desktop** | MCP Streamable HTTP + ai_* tools + passive scan(rayon) + Sequencer + WS/JWT tools + **snare-desktop(Tauri)** | agent หา IDOR ใน lab ได้เอง, remote MCP ต่อได้, desktop RAM ~30–50MB |
...
**เผื่อ scale:** เพราะ frontends คุยผ่าน `snare-client`/API, storage/engine อยู่หลัง port — เปลี่ยน SQLite→Postgres หรือเพิ่ม node ได้โดยไม่แตะ UI; core เป็น library ฝังในโหมด single-binary ก็ได้ กระจายเป็น daemon ก็ได้.
...
| 020 | Branding = **ตรวจชื่อก่อนลงทุน** (SNARE ชนของเดิม) | ลุยชื่อเดิม | เลี่ยง trademark/SEO collision (§45) |
...
- ช่องทาง: GitHub Releases (signed), Homebrew tap, AUR, `.deb`/`.rpm`, Docker image (`snared` headless), `cargo install`.
...
- **เทียบคู่แข่ง:** Snare vs Burp vs Caido vs ZAP บน workload เดียวกัน (idle RAM, RAM ที่ 100k flows, startup time, added-latency, intruder 10k req เวลาเสร็จ).
...
- **ผูก Stealth-Browser MCP** (ที่เรามีอยู่) — "replay ผ่าน browser จริง" ทะลุ anti-bot; และส่ง traffic จาก browser เข้า Snare.
...
Burp เก่งเพราะต่อได้ทุกที่ — Snare ต้องไม่แพ้. ทั้งหมดอยู่ที่ `snare-engine` + config (§29).
...
- **Upstream proxy**: ต่อออกผ่าน HTTP/HTTPS/**SOCKS5** upstream — ใช้ทะลุ corporate proxy, ยิงผ่าน **Tor**, หรือ chain **Snare → Burp/ZAP**.
...
| ปลายทาง | วิธี |
|---|---|
| **หน้า self-serve** | เปิด `http://snare.setup/` (หรือ `cert.snare`) ขณะต่อ proxy → ปุ่มดาวน์โหลด + คู่มือ per-OS |
...
- **Format**: **TOML** (comment ได้), โครง `snare.toml`.
- **ที่อยู่ (XDG)**: user `~/.config/snare/`, project `<project>/snare.toml`, state `~/.local/state/snare/`, cache `~/.cache/snare/`.
- **Precedence** [ADR 012]: **CLI flag > env (`SNARE_*`) > project config > user config > built-in default**.
...
- **CI/CD**: `snared` headless รับ OpenAPI → scan → ออก **SARIF + exit code** ให้ pipeline fail; เทียบ baseline กัน regression.
...
|---|---|---|
| **Single-binary** | TUI/Desktop ฝัง `snare-core` (ไม่มี daemon แยก, §3) | เครื่องเดียว, เบาสุด |
| **Daemon + clients** | `snared` + TUI/Web/Desktop ต่อผ่าน `snare-client` | รัน daemon บนเซิร์ฟเวอร์/VPS, คุมจากไกล (แบบ Caido) |
| **Headless / CI** | `snared` no-UI + REST/MCP + SARIF out (§34) | pipeline, scheduled scan |
| **Docker / K8s** | image `snared` headless, volume = project/blob | lab, team infra |
| **Team server** | `snared` + **Postgres** + RBAC/CRDT (§36) | หลายคน, shared engagement |
...
- **"SNARE" ชนของเดิม**: (ก) *SNARE* = Linux **audit/log agent** (InterSect Alliance) และ (ข) *SNARE* = **web application honeypot** (MushMush/Glastopf lineage) — ทั้งคู่อยู่สาย security ⇒ เสี่ยง **trademark + SEO + สับสน**.
- **ก่อนลงทุนโลโก้/โดเมน/แพ็กเกจ**: เช็ก crates.io/npm/GitHub org/โดเมน (`.io`/`.dev`) + trademark เบื้องต้น.
- ทางเลือก: คงชื่อแต่ทำ wordmark เฉพาะ (`Snare Proxy`/`snare-sec`), หรือ shortlist ชื่อสำรอง. **ตัดสินก่อน Phase 1** (ก่อนมี public artifact).

---

## 46. What Makes Snare Win (North-Star — ทำไมถึงชนะ ไม่ใช่แค่ตาม)

parity อย่างเดียวไม่พอ. สิ่งที่ Burp **เป็นไม่ได้** เพราะ legacy/ปิดโค้ด/Java คือสนามที่เราชนะ:

| แกนชนะ | Snare | Burp (ข้อจำกัดเชิงโครง) |
|---|---|---|
...
Burp เริ่มที่ traffic; Snare เริ่มได้ที่ **แค่ชื่อโดเมน**.
...
- **Headless ใน pipeline**: `snared` + schema → **SARIF + exit code** (§34) → PR fail ถ้ามี high; baseline suppression.
...
ต่อยอดนิสัยทำ **writeup** ของเรา — ให้ Snare เป็นทั้งอาวุธและครู.
...
## 54. Cloud / Hosted Snare (vision, optional — เฟสไกล)

- **Self-host team server** (§43) → ต่อยอดเป็น **managed cloud** (open-core §44): project ในคลาวด์, collaborate, agent รันบน worker.
- **Elastic scan workers**: กระจาย active scan/intruder ข้าม node (queue + `snare-core` เป็น library).
