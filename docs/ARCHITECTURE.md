...
| หลักการ | ทำจริงยังไง |
|---|---|
| **Ports & adapters** | logic อยู่ใน `snare-core` หลัง trait; storage/engine สลับ impl ได้ (SQLite↔Postgres) โดยไม่แตะ caller |
...
```mermaid
flowchart LR
  B["Browser / curl / tool"] -->|"proxy :8888"| E["snare-engine<br/>(data plane, hudsucker)"]
  E -->|"MITM TLS"| T["Target site"]
  E --> C["snare-core<br/>(intercept · rules · scanner ·<br/>session · ws · plugins)"]
...
```
crates/
├── snare-core           # โมเดล + trait + coordinators (ไม่มี I/O ของ proxy)
...
├── snare-store-sqlite   # FlowStore ← SQLite (Mutex<Connection>)   [local]
├── snare-store-postgres # FlowStore ← Postgres (DB-actor thread)   [team]
├── snare-engine         # data plane: hudsucker (hyper+rustls) + capture + hooks
├── snare-plugin         # WASM plugin host (wasmtime component model)
├── snared               # the daemon: wires engine + REST/SSE API + auth + config + pubsub
├── snare-tui            # ratatui frontend (REST+SSE client)
├── snare-desktop        # Tauri native window → dashboard
└── snare-mcp            # stdio MCP server (AI ขับผ่าน API)
```

**กฎการพึ่งพา:** `snare-core` ไม่รู้จัก proxy/HTTP/DB; ทุกอย่างพึ่ง core; adapter (store/engine/plugin)
พึ่ง core แล้ว `snared` ประกอบทั้งหมด. → เปลี่ยน backend หรือเพิ่ม frontend ไม่กระเทือนแกน.
...
- **Data plane** (`snare-engine`) — เส้นทางที่ traffic จริงวิ่ง. ต้องเร็ว, ต่อ connection. hudsucker
  รับ connection, MITM, เรียก handler ต่อ request/response.
- **Control plane** (`snared` REST/SSE) — คน/AI สั่งงาน (intercept toggle, add rule, run intruder).
...
```mermaid
flowchart TB
  subgraph DP["Data plane — snare-engine"]
    H["CaptureHandler<br/>(per connection)"]
  end
  subgraph CP["Control plane — snared axum"]
    API["REST /api/v1/*"]
    SSE["SSE /stream"]
  end
  subgraph SH["Shared (Arc) — snare-core"]
...
หัวใจอยู่ที่ `CaptureHandler::handle_request` / `handle_response` (`snare-engine/src/lib.rs`).
...
sequenceDiagram
  ...
  Px-->>Br: TLS cert ที่ Px ออกเอง (ลงนามด้วย Snare CA)
...
`snare-engine` ใช้ **[hudsucker]** (hyper + rustls + rcgen) เป็น data plane:

1. รับ `CONNECT site:443` → ตอบ `200` → เริ่ม TLS handshake กับ browser **โดยสวมเป็น site**.
2. hudsucker **ออก leaf cert ของ `site` สดๆ** (rcgen) ลงนามด้วย **Snare CA** ของเครื่อง, cache ไว้.
3. browser ทำ TLS handshake สำเร็จ (เพราะ browser trust Snare CA — ดู 5.3) → คุยเป็น plaintext กับ proxy.
...
Snare สร้าง **CA เฉพาะเครื่อง** (`snared ca generate` → `~/.config/snare/ca/snare-ca.pem` +
`.key` perms 0600). leaf cert ที่ proxy ออกให้ทุก site ลงนามด้วย CA นี้.
browser จะ trust ก็ต่อเมื่อ **ติดตั้ง `snare-ca.pem` ใน trust store**:
...
`config.json` (local) หรือตาราง `snare_settings` (team, แชร์กัน). อ่านตอน startup, save หลังทุก mutation.
...
flowchart LR
  subgraph DA["daemon A"]
    LA(("local bus")) -->|publish +origin=A| PGN[("NOTIFY snare_events")]
...
`snare-plugin` = **wasmtime component-model host**. Contract ผ่าน **WIT** (`wit/world.wit`):
plugin export `hooks` (name/on-request/on-response), host export `log` capability.

```mermaid
flowchart LR
  subgraph H["snare-plugin (host)"]
...
| **API/team auth** | project token → session token (getrandom, constant-time), middleware, idle 12h, logout, secrets ผ่าน env (`SNARE_AUTH_TOKEN`) ไม่โผล่ process list |
...
snared run                                  # proxy :8888 · dashboard :9000

# Team (Postgres + auth) — ใช้ env สำหรับ secrets
SNARE_POSTGRES=postgres://snare:snare@host/snare \
SNARE_AUTH_TOKEN=SECRET snared run

# เปลี่ยนพอร์ต
snared run --proxy 127.0.0.1:9999 --api 127.0.0.1:9001

# เครื่องมืออื่น
snared ca generate | ca path                # CA
snared browser --url https://target         # managed browser
snared flows | flush                        # CLI
snare-tui [--api https://.. --token ..]     # TUI (รองรับ TLS+auth)
snare-desktop                               # native window
snare-mcp                                   # MCP (env SNARE_API/SNARE_TOKEN/SNARE_AGENT)
```

**ไฟล์/พอร์ต** (override ทั้งหมดด้วย `SNARE_HOME`):
- `~/.config/snare/ca/` — CA · `~/.config/snare/config.json` — settings · `~/.config/snare/plugins/*.wasm`
- `~/.local/share/snare/flows.sqlite` — flows (local)
