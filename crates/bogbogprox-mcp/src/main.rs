//! `bogbogprox-mcp` — Model Context Protocol server over stdio (§10).
//!
//! Exposes BogBogProx's captured flows to AI agents. Phase-0 transport is a small
...
use bogbogprox_core::model::{Flow, FlowSummary};
...
impl ApiClient {
    fn from_env() -> Result<Self> {
        let base = std::env::var("BOGBOGPROX_API")
            .unwrap_or_else(|_| "http://127.0.0.1:9000".into())
            .trim_end_matches('/')
            .to_string();
        reqwest::Url::parse(&base).context("invalid BOGBOGPROX_API URL")?;
        let token = std::env::var("BOGBOGPROX_TOKEN").ok().filter(|v| !v.is_empty());
    ...
    fn notify_activity(&self, tool: &str, detail: &str) {
        let agent = std::env::var("BOGBOGPROX_AGENT").unwrap_or_else(|_| "ai-agent".into());
        let body = json!({
        ...
        if let Err(e) = result {
            eprintln!("[bogbogprox-mcp] activity notification failed: {e}");
        }
    }
}
...
fn main() -> Result<()> {
    ...
    for line in stdin.lock().lines() {
        ...
        let msg: Value = match serde_json::from_str(&line) {
            Ok(v) => v,
            Err(e) => {
                eprintln!("[bogbogprox-mcp] bad json: {e}");
                continue;
            }
        };
        ...
        eprintln!("[bogbogprox-mcp] -> {method}");
...
fn handle(method: &str, params: Option<&Value>, api: &ApiClient) -> Result<Value> {
    match method {
        "initialize" => {
            api.notify_activity("connect", "AI agent connected to BogBogProx");
            Ok(json!({
                "protocolVersion": PROTOCOL_VERSION,
                "capabilities": { "tools": {} },
                "serverInfo": { "name": "bogbogprox-mcp", "version": env!("CARGO_PKG_VERSION") }
            }))
        }
        "ping" => Ok(json!({})),
...
fn tool_specs() -> Value {
    json!([
        ...
        {
            "name": "repeater_send",
            "description": "Resend a captured request through the repeater by flow id. Returns the new flow (request + response). Needs a running `bogbogproxd`.",
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        {
            "name": "intruder_run",
            "description": "Fuzz a captured request: substitute `marker` in its URL/headers/body with each payload and send them (bounded-parallel). Returns status/length per payload. Needs a running `bogbogproxd`.",
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        {
            "name": "active_scan",
            "description": "Active-scan a captured flow's query parameters for reflected-XSS and error-based SQLi. Returns per-probe results; findings appear in the dashboard. Needs a running `bogbogproxd`.",
            "inputSchema": {
                "type": "object",
...
fn format_flow(flow: &bogbogprox_core::model::Flow, part: &str) -> Value {
...
mod tests {
    use super::*;
    use bogbogprox_core::model::Source;
